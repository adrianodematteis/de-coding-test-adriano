...
import ai.humn.telematics.types.JourneyDetails
import ai.humn.telematics.utils.Utils
...
class ProcessDataFileTest extends FlatSpec with Matchers {
  ...
  it should "result in a JourneyDetails instance with no distance, no duration and no avg speed" in {
    val res =
      Utils.parseJourney("000008,driver_e,1633436902000,1633436902000,0.125,0.458,0.129,0.558,123360,123360")
  ...
  it should "result in a JourneyDetails instance with no duration and no avg speed" in {
    val res =
      Utils.parseJourney("000008,driver_e,1633437902000,1633436902000,0.125,0.458,0.129,0.558,123360,123361")
  ...
  it should "result in a JourneyDetails instance with no distance and no avg speed" in {
    val res =
      Utils.parseJourney("000008,driver_e,1633436902000,1633446902000,0.125,0.458,0.129,0.558,123360,123351")
  ...
  it should "result in a JourneyDetails instance with distance, duration and avg speed" in {
    val res =
      Utils.parseJourney("000001,driver_a,1633429702000,1633430302000,0.123,0.456,0.124,0.457,123456,123459")
